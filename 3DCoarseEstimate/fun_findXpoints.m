function [avg_norm, xpoint] = fun_findXpoints(phase, ncp, edgeCoords, cp, inode_in, nx, ny,contactPoints, R1finePix, minR, Allvectors)
...
if all(isfinite(ns))
    avg_norm = [mean(ns(:,1)), mean(ns(:,2)), mean(ns(:,3))];
end
